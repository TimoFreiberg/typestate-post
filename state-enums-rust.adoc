...
It has the same benefits as the states-as-enums approach (even partially improving on them) and removes the drawback - no need to have the cumbersome state check at the start of every method!

But first, let's talk a bit about why the version using enums is suboptimal for this algorithm.
...
But much of this overhead comes from the power to always change the `state` at any time - which isn't even needed!
Maybe making the interfaces more restrictive could make the code look nicer...
